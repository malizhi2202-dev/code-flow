...
**代码规模**：前端 74 个 tsx + 15 个 ts 文件 + 后端 97 个 py 文件。全栈 SPA 架构：React 18 + TypeScript 前端 → FastAPI 后端 → SQLite/MySQL。

> **源文档**：本次扫描综合了 code-kit 既有 CONTEXT.md + CLAUDE.md + 代码库 grep/find 取证。
...
## `fix-slow-access` 域语言（新增 · 2026-07-07）

| 术语 | 定义 |
|---|---|
| in-flight 去重（Request Dedup） | 同一 URL + params 的多个并发请求合并为一个，所有调用方共享同一个 Promise 结果 |
| 短期缓存（Short Cache） | Session 级内存缓存，key=URL+params，TTL=5s，页面切换不丢失，手动刷新可穿透 |
| 轮询泄漏（Polling Leak） | setInterval 在组件卸载后未清除，导致后台持续发送请求，跨页面累积 |
| 死亡螺旋（Death Spiral） | 前述问题的极端表现：长时间使用后 API 调用次数指数增长（7→149），最慢请求 > 100s |
...
- **Store 清单**（9 个文件，均在 `frontend/src/stores/`）：
...
| `chat.ts:1` | — | 对话中心消息/会话状态（`create<ChatState>`） |
| `orchestration.ts:1` | — | K8s 编排拓扑状态（`create<OrchestrationState>`） |
...
- **模型文件**（`backend/models/`，13 个）：`tool.py`、`workflow.py`、`agent.py`、`agent_memory.py`、`channel_config.py`、`conversation.py`、`knowledge_source.py`、`message.py`、`metrics.py`、`orchestration.py`、`project.py`、`role_custom.py`
- **新模块**（2026-07-05 扫描新增）：
  - `backend/engine/`：编排引擎（`gate_registry.py`、`reconcile_loop.py`、`scheduler.py`、`yaml_schema.py`）
  - `backend/storage/`：存储后端抽象（`memory_backend.py`、`sqlite_backend.py`、`mysql_backend.py`）
  - `backend/services/` 新增：`channel_adapter.py`、`chat_service.py`、`codekit_importer.py`、`gate_resolver.py`、`metrics_scheduler.py`、`oauth_*.py`（4 个）、`orchestration_parser.py`、`runtime_tracer.py`、`template_service.py`
  - `backend/routes/` 新增：`agent_knowledge_api.py`、`channel_api.py`、`chat_api.py`
...
- **测试文件**：✅ 已有测试文件。前端 `__tests__/` 5 个（`ChannelConfig.test.tsx`、`ChatWindow.test.tsx`、`ConversationCenter.test.tsx`、`chat-store.test.ts`、`frontend-edge-cases.test.tsx`），后端 `tests/` 7 个（`test_api_integration.py`、`test_edge_cases.py`、`test_agent_channel.py`、`test_agent_channel_supplement.py`、`test_round6_api_edges.py`、`test_round7_security.py`）
...
| 债项 | 来源 | 首次发现 | 影响 | 计划处理 |
|---|---|---|---|---|
|| 🔴 无业务测试文件（Vitest 已配置但 0 个测试） | 2026-07-03 intel-scan | 2026-07-03 | 重构/新增功能无回归保护 | ✅ 已解决（2026-07-05 扫描：前端 5 + 后端 7 个测试文件） |
...
- **last_intel_scan**: 2026-07-05
- **scanner**: `prompts/I-intel-scan.md`
- **detected_stack**: React 18 + TypeScript + Vite + Tailwind + Zustand | FastAPI + SQLAlchemy + SQLite/MySQL + Redis
- **source_docs_consulted**: `.specs/CONTEXT.md`（code-kit 既有）+ `CLAUDE.md` + `code-kit-monitor/README.md`
- **files_sampled**: `frontend/package.json`, `backend/requirements.txt`, `backend/main.py`, `backend/database.py`, `backend/auth.py`, `frontend/src/main.tsx`, `frontend/src/stores/*.ts`（9 个）, `frontend/src/hooks/*.ts`（2 个）, `frontend/src/components/*.tsx`（25 个）, `frontend/src/pages/*.tsx`（20 个）, `frontend/src/__tests__/*`（5 个）, `backend/models/*.py`（13 个）, `backend/routes/*.py`（24 个）, `backend/services/*.py`（20 个）, `backend/engine/*.py`（5 个）, `backend/storage/*.py`（4 个）, `backend/tests/*.py`（7 个）, `backend/runtime/adapters/*.py`（5 个）
- **code_scale**: 后端 97 .py | 前端 74 .tsx + 15 .ts
- **tests_exist**: ✅ 前端 5 + 后端 7
- **下次重扫建议**: 下次架构级变更后或距本次 > 90 天
