...
import { Plus, Play, Square, Trash2, FolderKanban, Upload, FileText } from 'lucide-react';
...
export default function ProjectManager({ onSelect }: Props) {
  ...
  const [form, setForm] = useState({ name: '', requirement_raw: '', requirement_type: 'text', agent_id: 0, workflow_id: 0 });
  ...
  const handleCreate = async () => {
    ...
    setShowCreate(false);
    setForm({ name: '', requirement_raw: '', requirement_type: 'text', agent_id: 0, workflow_id: 0 });
  };
            ...
            <div key={p.id} onClick={() => onSelect?.(p.id)} style={{ background: 'var(--color-surface)', borderRadius: 8, padding: 16, border: '1px solid var(--color-border)', cursor: onSelect ? 'pointer' : 'default' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  ...
                  {p.status !== 'running' && <button onClick={() => setDeleteId(p.id)} style={{ padding: 4, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-danger)' }} title="删除"><Trash2 size={14} /></button>}
                </div>
              </div>
          ...
          <div style={{ background: 'var(--color-elevated)', borderRadius: 8, padding: 24, width: 520, maxHeight: '85vh', overflow: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div><label style={lbl}>绑定 Agent</label><select value={form.agent_id} onChange={(e) => setForm({ ...form, agent_id: +e.target.value })} style={inp}><option value={0}>-- 选择 Agent --</option>{agents.map(a => <option key={a.id} value={a.id}>{a.name} ({a.model_name})</option>)}</select></div>
                <div><label style={lbl}>绑定工作流</label><select value={form.workflow_id} onChange={(e) => setForm({ ...form, workflow_id: +e.target.value })} style={inp}><option value={0}>-- 选择工作流 --</option>{workflows.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>
              </div>
