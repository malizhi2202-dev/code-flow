import { useEffect, useState } from 'react';
import {
  Settings,
  Save,
  RotateCcw,
  RefreshCw,
  Tag,
  Gauge,
  ChevronDown,
  ChevronRight,
  Play,
  Pause,
  Trash2,
  Plus,
} from 'lucide-react';
import { useControlPlane, type QueueItem } from '../stores/controlPlane';

// ── 调度算法选项 ──
const ALGORITHM_OPTIONS = [
  { value: 'round_robin', label: '轮询 (Round Robin)' },
  { value: 'least_connections', label: '最少连接 (Least Connections)' },
  { value: 'priority_based', label: '优先级调度 (Priority-Based)' },
  { value: 'weighted_fair', label: '加权公平 (Weighted Fair Queue)' },
  { value: 'random', label: '随机 (Random)' },
];

// ── 优先级选项 ──
const PRIORITY_OPTIONS = [
  { value: 'high', label: '🔴 高', color: '#dc2626' },
  { value: 'medium', label: '🟡 中', color: '#e8a450' },
  { value: 'low', label: '🟢 低', color: '#5cb878' },
];

// ── 全局调度配置接口 ──
interface SchedulerGlobalConfig {
  algorithm: string;
  retry_count: number;
  probe_interval: number;
  max_concurrency: number;
  queue_timeout: number;
}

// ── Agent 标签/优先级行 ──
interface AgentTagRow {
  agent_id: number;
  agent_name: string;
  tags: string;
  priority: string;
  concurrency: number;
  enabled: boolean;
}

/**
 * SchedulerConfig — 管理调度配置面板
 * - 上半部分：全局调度参数表单（算法 / 重试次数 / 探针间隔 / 并发上限 / 超时）
 * - 下半部分：Agent 标签与优先级表格（可编辑标签、优先级下拉、并发数）
 */
export default function SchedulerConfig() {
  const { queue, loading, fetchQueue } = useControlPlane();

  // 全局配置
  const [globalConfig, setGlobalConfig] = useState<SchedulerGlobalConfig>({
    algorithm: 'priority_based',
    retry_count: 3,
    probe_interval: 30,
    max_concurrency: 10,
    queue_timeout: 300,
  });
  const [configDirty, setConfigDirty] = useState(false);

  // Agent 表格行（从 queue 数据推导）
  const [agentRows, setAgentRows] = useState<AgentTagRow[]>([]);
  const [editingCell, setEditingCell] = useState<{ rowIdx: number; field: string } | null>(null);
  const [expanded, setExpanded] = useState(false);

  useEffect(() => {
    fetchQueue();
  }, [fetchQueue]);

  // 从 queue 推演出 agent 表格行
  useEffect(() => {
    const agentMap = new Map<number, AgentTagRow>();
    for (const item of queue) {
      if (!agentMap.has(item.agent_id)) {
        agentMap.set(item.agent_id, {
          agent_id: item.agent_id,
          agent_name: item.agent_name,
          tags: '',
          priority: item.priority >= 7 ? 'high' : item.priority >= 4 ? 'medium' : 'low',
          concurrency: 1,
          enabled: item.status !== 'disabled',
        });
      }
    }
    setAgentRows([...agentMap.values()]);
  }, [queue]);

  // ── 全局配置变更处理 ──
  const updateGlobal = (field: keyof SchedulerGlobalConfig, value: string | number) => {
    setGlobalConfig((prev) => ({ ...prev, [field]: value }));
    setConfigDirty(true);
  };

  const saveGlobalConfig = () => {
    const uid = localStorage.getItem('current_user_id') || 'admin';
    fetch('/api/scheduler/config', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-User-Id': uid },
      body: JSON.stringify(globalConfig),
    })
      .then((r) => r.json())
      .then(() => setConfigDirty(false))
      .catch(() => {});
  };

  const resetGlobalConfig = () => {
    setGlobalConfig({
      algorithm: 'priority_based',
      retry_count: 3,
      probe_interval: 30,
      max_concurrency: 10,
      queue_timeout: 300,
    });
    setConfigDirty(true);
  };

  // ── Agent 行变更处理 ──
  const updateAgentRow = (idx: number, field: keyof AgentTagRow, value: any) => {
    setAgentRows((prev) => {
      const next = [...prev];
      next[idx] = { ...next[idx], [field]: value };
      return next;
    });
  };

  const saveAgentRow = (idx: number) => {
    const row = agentRows[idx];
    const uid = localStorage.getItem('current_user_id') || 'admin';
    fetch(`/api/scheduler/agents/${row.agent_id}/tags`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-User-Id': uid },
      body: JSON.stringify({
        tags: row.tags.split(',').map((t) => t.trim()).filter(Boolean),
        priority: row.priority,
        concurrency: row.concurrency,
        enabled: row.enabled,
      }),
    }).catch(() => {});
  };

  const toggleAgent = (idx: number) => {
    updateAgentRow(idx, 'enabled', !agentRows[idx].enabled);
  };

  // ── 样式常量 ──
  const panel: React.CSSProperties = {
    background: 'var(--bg-card, #181a1f)',
    borderRadius: 8,
    padding: 16,
    border: '1px solid var(--border-normal, #2a2d35)',
    marginBottom: 12,
  };
  const sectionTitle: React.CSSProperties = {
    fontSize: 13,
    fontWeight: 600,
    margin: '0 0 12px 0',
    color: 'var(--color-text)',
  };
  const inp: React.CSSProperties = {
    width: '100%',
    padding: '6px 10px',
    background: 'var(--bg-input, #1e2130)',
    color: 'var(--color-text)',
    border: '1px solid var(--color-border, #2a2d35)',
    borderRadius: 4,
    fontSize: 11,
    boxSizing: 'border-box',
    fontFamily: 'var(--font-mono, monospace)',
  };
  const selectStyle: React.CSSProperties = { ...inp, cursor: 'pointer' };
  const lbl: React.CSSProperties = {
    fontSize: 10,
    fontWeight: 500,
    color: 'var(--text-dim)',
    display: 'block',
    marginBottom: 4,
  };

  if (queue.length === 0 && loading) {
    return (
      <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-dim)', fontSize: 12 }}>
        <RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> 加载中...
      </div>
    );
  }

  return (
    <div>
      {/* ═══════════════════════════════════════
          全局调度配置
          ═══════════════════════════════════════ */}
      <div style={panel}>
        <div
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}
        >
          <h3 style={{ ...sectionTitle, margin: 0 }}>
            <Settings size={16} style={{ verticalAlign: -3, marginRight: 6 }} />
            全局调度配置
          </h3>
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              onClick={resetGlobalConfig}
              style={{
                padding: '4px 10px',
                fontSize: 10,
                background: 'none',
                border: '1px solid var(--color-border, #2a2d35)',
                borderRadius: 4,
                cursor: 'pointer',
                color: 'var(--text-dim)',
                display: 'flex',
                alignItems: 'center',
                gap: 4,
              }}
            >
              <RotateCcw size={11} /> 重置
            </button>
            <button
              onClick={saveGlobalConfig}
              disabled={!configDirty}
              style={{
                padding: '4px 14px',
                fontSize: 10,
                background: configDirty ? 'var(--color-primary, #3b82f6)' : 'var(--bg-input, #1e2130)',
                color: configDirty ? '#fff' : 'var(--text-dim)',
                border: 'none',
                borderRadius: 4,
                cursor: configDirty ? 'pointer' : 'default',
                display: 'flex',
                alignItems: 'center',
                gap: 4,
              }}
            >
              <Save size={11} /> 保存配置
            </button>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {/* 算法 */}
          <div>
            <label style={lbl}>
              <Gauge size={10} style={{ verticalAlign: -1, marginRight: 4 }} />
              调度算法
            </label>
            <select
              value={globalConfig.algorithm}
              onChange={(e) => updateGlobal('algorithm', e.target.value)}
              style={selectStyle}
            >
              {ALGORITHM_OPTIONS.map((opt) => (
                <option key={opt.value} value={opt.value}>
                  {opt.label}
                </option>
              ))}
            </select>
          </div>

          {/* 重试次数 */}
          <div>
            <label style={lbl}>重试次数</label>
            <input
              type="number"
              min={0}
              max={10}
              value={globalConfig.retry_count}
              onChange={(e) => updateGlobal('retry_count', parseInt(e.target.value) || 0)}
              style={inp}
            />
          </div>

          {/* 探针间隔 */}
          <div>
            <label style={lbl}>探针间隔 (秒)</label>
            <input
              type="number"
              min={5}
              max={600}
              value={globalConfig.probe_interval}
              onChange={(e) => updateGlobal('probe_interval', parseInt(e.target.value) || 30)}
              style={inp}
            />
          </div>

          {/* 最大并发 */}
          <div>
            <label style={lbl}>最大并发数</label>
            <input
              type="number"
              min={1}
              max={100}
              value={globalConfig.max_concurrency}
              onChange={(e) => updateGlobal('max_concurrency', parseInt(e.target.value) || 1)}
              style={inp}
            />
          </div>

          {/* 队列超时 */}
          <div>
            <label style={lbl}>队列超时 (秒)</label>
            <input
              type="number"
              min={30}
              max={3600}
              value={globalConfig.queue_timeout}
              onChange={(e) => updateGlobal('queue_timeout', parseInt(e.target.value) || 300)}
              style={inp}
            />
          </div>
        </div>

        {/* 当前队列统计 */}
        <div
          style={{
            display: 'flex',
            gap: 16,
            marginTop: 14,
            padding: '8px 12px',
            background: 'var(--bg-input, #1e2130)',
            borderRadius: 6,
            fontSize: 10,
            color: 'var(--text-dim)',
          }}
        >
          <span>📋 队列任务：<b style={{ color: 'var(--color-text)' }}>{queue.length}</b></span>
          <span>
            🟢 待处理：<b style={{ color: '#5cb878' }}>{queue.filter((q) => q.status === 'pending').length}</b>
          </span>
          <span>
            🔵 处理中：<b style={{ color: '#3b82f6' }}>{queue.filter((q) => q.status === 'running').length}</b>
          </span>
          <span>
            🔴 失败：<b style={{ color: '#dc2626' }}>{queue.filter((q) => q.status === 'failed').length}</b>
          </span>
        </div>
      </div>

      {/* ═══════════════════════════════════════
          Agent 标签与优先级表格
          ═══════════════════════════════════════ */}
      <div style={panel}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ ...sectionTitle, margin: 0 }}>
            <Tag size={16} style={{ verticalAlign: -3, marginRight: 6 }} />
            Agent 标签与优先级
          </h3>
          <button
            onClick={() => fetchQueue()}
            style={{
              padding: '4px 10px',
              fontSize: 10,
              background: 'none',
              border: '1px solid var(--color-border, #2a2d35)',
              borderRadius: 4,
              cursor: 'pointer',
              color: 'var(--text-secondary)',
              display: 'flex',
              alignItems: 'center',
              gap: 4,
            }}
          >
            <RefreshCw size={11} /> 刷新
          </button>
        </div>

        {agentRows.length === 0 ? (
          <div style={{ fontSize: 11, color: 'var(--text-dim)', textAlign: 'center', padding: 20 }}>
            暂无 Agent 数据（队列为空）
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--color-border, #2a2d35)' }}>
                  <th style={th}>Agent</th>
                  <th style={th}>标签</th>
                  <th style={th}>优先级</th>
                  <th style={th}>并发数</th>
                  <th style={{ ...th, textAlign: 'center' }}>启/停</th>
                  <th style={{ ...th, textAlign: 'center' }}>保存</th>
                </tr>
              </thead>
              <tbody>
                {agentRows.map((row, idx) => (
                  <tr
                    key={row.agent_id}
                    style={{
                      borderBottom: '1px solid var(--color-border, #2a2d35)',
                      opacity: row.enabled ? 1 : 0.5,
                    }}
                  >
                    {/* Agent 名称 */}
                    <td style={td}>
                      <span style={{ fontWeight: 500 }}>{row.agent_name}</span>
                      <div style={{ fontSize: 9, color: 'var(--text-dim)' }}>ID: {row.agent_id}</div>
                    </td>

                    {/* 可编辑标签 */}
                    <td style={td}>
                      {editingCell?.rowIdx === idx && editingCell?.field === 'tags' ? (
                        <input
                          autoFocus
                          value={row.tags}
                          onChange={(e) => updateAgentRow(idx, 'tags', e.target.value)}
                          onBlur={() => setEditingCell(null)}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') setEditingCell(null);
                            if (e.key === 'Escape') setEditingCell(null);
                          }}
                          placeholder="tag1, tag2, ..."
                          style={{
                            ...inp,
                            width: 120,
                            padding: '3px 6px',
                            fontSize: 10,
                          }}
                        />
                      ) : (
                        <div
                          onClick={() => setEditingCell({ rowIdx: idx, field: 'tags' })}
                          style={{
                            cursor: 'pointer',
                            minHeight: 24,
                            display: 'flex',
                            flexWrap: 'wrap',
                            gap: 4,
                            alignItems: 'center',
                          }}
                          title="点击编辑标签"
                        >
                          {row.tags
                            ? row.tags.split(',').map((t) => t.trim()).filter(Boolean).map((tag) => (
                                <span
                                  key={tag}
                                  style={{
                                    fontSize: 9,
                                    padding: '1px 8px',
                                    background: 'rgba(168,85,247,0.15)',
                                    color: '#c084fc',
                                    borderRadius: 8,
                                  }}
                                >
                                  {tag}
                                </span>
                              ))
                            : (
                              <span style={{ fontSize: 10, color: 'var(--text-dim)', fontStyle: 'italic' }}>
                                无标签（点击编辑）
                              </span>
                            )}
                        </div>
                      )}
                    </td>

                    {/* 优先级下拉 */}
                    <td style={td}>
                      <select
                        value={row.priority}
                        onChange={(e) => updateAgentRow(idx, 'priority', e.target.value)}
                        style={{
                          ...selectStyle,
                          width: 100,
                          padding: '3px 6px',
                          fontSize: 10,
                          color:
                            row.priority === 'high'
                              ? '#dc2626'
                              : row.priority === 'medium'
                              ? '#e8a450'
                              : '#5cb878',
                        }}
                      >
                        {PRIORITY_OPTIONS.map((p) => (
                          <option key={p.value} value={p.value}>
                            {p.label}
                          </option>
                        ))}
                      </select>
                    </td>

                    {/* 并发数 */}
                    <td style={td}>
                      <input
                        type="number"
                        min={1}
                        max={20}
                        value={row.concurrency}
                        onChange={(e) =>
                          updateAgentRow(idx, 'concurrency', parseInt(e.target.value) || 1)
                        }
                        style={{
                          ...inp,
                          width: 60,
                          padding: '3px 6px',
                          fontSize: 10,
                          textAlign: 'center',
                        }}
                      />
                    </td>

                    {/* 启/停 */}
                    <td style={{ ...td, textAlign: 'center' }}>
                      <button
                        onClick={() => toggleAgent(idx)}
                        title={row.enabled ? '点击停用' : '点击启用'}
                        style={{
                          padding: '3px 8px',
                          fontSize: 10,
                          border: 'none',
                          borderRadius: 3,
                          cursor: 'pointer',
                          background: row.enabled
                            ? 'rgba(92,184,120,0.12)'
                            : 'rgba(107,114,128,0.15)',
                          color: row.enabled ? '#5cb878' : '#6b7280',
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: 3,
                        }}
                      >
                        {row.enabled ? <Play size={10} /> : <Pause size={10} />}
                        {row.enabled ? '运行中' : '已停用'}
                      </button>
                    </td>

                    {/* 保存行 */}
                    <td style={{ ...td, textAlign: 'center' }}>
                      <button
                        onClick={() => saveAgentRow(idx)}
                        style={{
                          padding: '3px 10px',
                          fontSize: 10,
                          background: 'var(--color-primary, #3b82f6)',
                          color: '#fff',
                          border: 'none',
                          borderRadius: 3,
                          cursor: 'pointer',
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: 3,
                        }}
                      >
                        <Save size={10} /> 保存
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}

        {/* 展开查看更多队列 */}
        <div
          onClick={() => setExpanded(!expanded)}
          style={{
            marginTop: 10,
            padding: '6px 10px',
            cursor: 'pointer',
            display: 'flex',
            alignItems: 'center',
            gap: 4,
            fontSize: 11,
            color: 'var(--text-dim)',
            background: 'var(--bg-input, #1e2130)',
            borderRadius: 4,
          }}
        >
          {expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
          队列详情（{queue.length} 项）
        </div>

        {expanded && queue.length > 0 && (
          <div style={{ marginTop: 8, maxHeight: 200, overflowY: 'auto' }}>
            {queue.map((item) => (
              <div
                key={item.id}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
                  padding: '6px 10px',
                  borderBottom: '1px solid var(--color-border, #2a2d35)',
                  fontSize: 10,
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span
                    style={{
                      width: 8,
                      height: 8,
                      borderRadius: '50%',
                      display: 'inline-block',
                      background:
                        item.status === 'pending'
                          ? '#9ca3af'
                          : item.status === 'running'
                          ? '#3b82f6'
                          : item.status === 'completed'
                          ? '#5cb878'
                          : '#dc2626',
                    }}
                  />
                  <span style={{ color: 'var(--color-text)' }}>{item.orchestration_name}</span>
                  <span style={{ color: 'var(--text-dim)' }}>
                    → {item.agent_name}
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ color: 'var(--text-dim)' }}>
                    优先级: {item.priority}
                  </span>
                  <span
                    style={{
                      fontSize: 9,
                      fontFamily: 'var(--font-mono, monospace)',
                      color: 'var(--text-dim)',
                    }}
                  >
                    {item.created_at || '-'}
                  </span>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}

// ── 表格样式 ──
const th: React.CSSProperties = {
  padding: '8px 10px',
  fontSize: 10,
  color: 'var(--text-dim)',
  fontWeight: 500,
  textTransform: 'uppercase',
  textAlign: 'left',
  whiteSpace: 'nowrap',
};

const td: React.CSSProperties = {
  padding: '8px 10px',
  fontSize: 11,
  color: 'var(--color-text)',
  verticalAlign: 'middle',
};
