...
interface ChatState {
  ...
  createConversation: (agentId: number) => Promise<Conversation | null>;
  sendMessage: (content: string) => Promise<void>;
  loadMessages: (sinceId?: number) => void;
  pollMessages: () => void;
  clearError: () => void;
}
...
export const useChat = create<ChatState>((set, get) => ({
  ...
  createConversation: async (agentId: number) => {
    try {
  ...
  sendMessage: async (content: string) => {
    ...
    try {
      ...
      if (data.ok) {
        const newMsgs: ChatMessage[] = [data.user_message, data.agent_message].filter(Boolean);
        set(s => ({
