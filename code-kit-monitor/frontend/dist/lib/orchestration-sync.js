...
export function defaultEdgeConfig(id, source, target) {
    return {
        id, source, target,
        type: 'sequential',
        description: '',
...
export function agentNameToId(name) {
    return `agent-${name.replace(/[^a-zA-Z0-9一-鿿_-]/g, '_')}`;
}
export function yamlToTopology(yamlText) {
    const nodes = [];
    const edges = [];
    const edgeConfigs = new Map();
    if (!yamlText.trim())
    ...
    routes.forEach((route, i) => {
        ...
        edgeConfigs.set(edgeId, {
            ...
            trigger_type: route.trigger_type || 'auto',
...
// Topology → YAML
// ═══════════════════════════════════════════
export function topologyToYaml(nodes, edges, edgeConfigs, name = 'my-orchestration') {
    const agents = nodes
...
// YAML ↔ MD
// ═══════════════════════════════════════════
export function yamlToMd(yamlText) {
    const doc = parseYamlSafe(yamlText);
    if (!doc)
        return `# 编排\n\n> 无法解析 YAML`;
...
export function mdToYaml(mdText) {
...
function dumpYamlSafe(obj) {
    ...
    for (const a of obj.spec?.agents || []) {
        ...
        if (a.position)
            y += `        position: {x: ${a.position.x}, y: ${a.position.y}}\n`;
    }
    y += '  routes:\n';
    for (const r of obj.spec?.routes || []) {
        y += `    - from: ${r.from}\n      to: ${r.to}\n      type: ${r.type || 'sequential'}\n`;
        if (r.trigger_condition)
            y += `      trigger_condition: "${r.trigger_condition}"\n`;
        if (r.trigger_type)
            y += `      trigger_type: ${r.trigger_type}\n`;
        if (r.description)
