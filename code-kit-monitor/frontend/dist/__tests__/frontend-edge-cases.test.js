...
describe('ChatWindow — 异常操作', () => {
    ...
    it('prevents double-send while loading', () => {
        const onSend = vi.fn();
        const { rerender } = render(_jsx(ChatWindow, { agentId: 1, agentName: "A", messages: [], loading: true, error: null, onSend: onSend }));
    ...
    it('trims and rejects whitespace-only messages', () => {
        const onSend = vi.fn();
        render(_jsx(ChatWindow, { agentId: 1, agentName: "A", messages: [], loading: false, error: null, onSend: onSend }));
    ...
    it('renders 100 messages without crash', () => {
        ...
        render(_jsx(ChatWindow, { agentId: 1, agentName: "A", messages: messages, loading: false, error: null, onSend: vi.fn() }));
    ...
    it('renders very long agent name', () => {
        ...
        render(_jsx(ChatWindow, { agentId: 1, agentName: longName, messages: [], loading: false, error: null, onSend: vi.fn() }));
    ...
    it('clears error and allows retry', () => {
        const onRetry = vi.fn();
        const onSend = vi.fn();
        const { rerender } = render(_jsx(ChatWindow, { agentId: 1, agentName: "A", messages: [], loading: false, error: "Agent \u6682\u65F6\u65E0\u6CD5\u56DE\u590D", onSend: onSend, onRetry: onRetry }));
