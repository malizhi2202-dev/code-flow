...
export function dedupedFetch(url, options) {
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
        if (IS_DEV)
            console.debug(`[requestDedup] cache hit: ${key}`);
    ...
    if (flying) {
        if (IS_DEV)
            console.debug(`[requestDedup] in-flight hit: ${key}`);
        return flying.then(r => r.clone());
    }
    // 3. 新建请求
    if (IS_DEV)
        console.debug(`[requestDedup] new fetch: ${key}`);
...
export function invalidateCache(url) {
    if (url) {
        ...
        cache.delete(key);
        if (IS_DEV)
            console.debug(`[requestDedup] cache invalidated: ${key}`);
    }
    else {
        cache.clear();
        if (IS_DEV)
            console.debug('[requestDedup] all cache cleared');
    }
}
