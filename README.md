...
**🏗️ Agent 管控平台**（新增 — K8s 架构思想）
- 隔离域（Namespace）：域内 Agent 自动发现，跨域默认隔离
- Capability 分组（Deployment）：同能力 Agent 自动成组，显示健康/总数
- 自动路由（Service）：标签匹配 + Least Connection 负载均衡，选最空闲 Agent
- 网关（Ingress）：统一入口，按 capability 标签路由到目标 Agent
- 渐进式扩容（HPA）：排队超阈值自动创建副本
- 记忆系统（etcd）：新 Agent 启动自动加载同域上下文
- 全栈监控：主机(CPU/内存/磁盘) + Agent(Token/耗时) + 模型(Rate Limit)
- 自愈修复：Reconcile Loop 三级修复（safe/caution/dangerous）
...
**代码规模**：后端 105+ .py + 前端 95+ ts/tsx + 15+ 测试文件
